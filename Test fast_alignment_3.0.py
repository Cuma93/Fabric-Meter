...
def regression_perform(points_coordinates, img):
    ...
    return round(res.slope, 3), res.intercept
...
# Individua a video i fori contati due volte e li corregge.
...
def alignment_counting(max_holes, min_dist):
    global hole_cascade
    hole_cascade = cv2.CascadeClassifier('/home/pi/Desktop/Fabric-Meter/hole classifier 2.0/classifier/hole_cascade_2.0.xml')
    state_check = False
    center_needles = 480/2 #..............................................Settaggio del centro delle bobine in direz. verticale
    static_sup_lim = center_needles - 45 #............................................Limite superiore fascia di controllo permanente
    static_inf_lim = center_needles + 45 #............................................Limite inferiore fascia di controllo permanente
    counter = 0 # contatore di cicli    
    aligner_position_counter = pos[2]
    video_position_counter = pos[3]
    last_gap = round((640 / max_holes * (max_holes - 1)) * 3.19444444444) + 600 # Limite di step che indica che abbiamo raggiunto il penultimo settore con la videocamera.
    j = 1
    finisher = False
    holes_counter = 0
    ...
    while (GPIO.input(proxy_videocamera) == True):
        ...
        dynamyc_sup_lim = first_point[1] - 30
        dynamic_inf_lim = first_point[1] + 30
        _, _, _, last_point, row_coordinates = middleR_perform(frame_color, coordinates, dynamyc_sup_lim, dynamic_inf_lim) # Restituisce il vettore di punti nella fascia di controllo e le coordinate dei punti ordinati
        ...
        cv2.circle(frame_color,(int(last_point[0]), int(last_point[1])), 5, (0, 255, 255), 2) #................................Evidenzia l'ultimo foro della riga
        #cv2.line(frame_color,(0, int(dynamic_sup_lim)) , (640, int(dynamic_sup_lim)),(255, 0, 255) ,1)  # linea superiore (magenta)
        #cv2.line(frame_color,(0, int(dynamic_sup_lim)) , (640, int(dynamic_sup_lim)),(255, 0, 255) ,1)  # linea inferiore (magenta)
        print("Coefficiente angolare. " + str(m))
        # Inizializza l'output video solo il primo ciclo
        ...
        if (- 0.015 < m) and (m < 0.015):
                thresholded_row_img, _ = best_filtering_row(frame, _, dynamyc_sup_lim, dynamic_inf_lim)
                coordinates, _, frame_color = detect_hole(thresholded_row_img)
                ret, doubled_corrected_coordinates = doubled(coordinates, min_dist, frame_color)
                ret, missed_corrected_coordinates = missed(doubled_corrected_coordinates, min_dist, frame_color)
                holes_counter = holes_counter + len(missed_corrected_coordinates)
                ...
                if (pos[1] <= last_gap): 
                    print("Raggiunto il penultimo settore")
                    distance_x = last_shift(row_coordinates, max_holes, min_dist)
                    finisher = True

                feeding_video = round(distance_x * 3.19444444444) # traduce i pixels in avanzamento motore considerando motori con 800steps/giro e rapporto di ingranaggio specifico
                video_position_counter = video_position_counter - feeding_video
                print(74400 - video_position_counter)
                cv2.imshow("Alignment", frame_color)
...
def start():
    if (check_setting == True):
        ...
        check_alignment_counting = alignment_counting(6, 85.7)
